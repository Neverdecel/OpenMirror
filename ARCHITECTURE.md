...
The design contract for Relit: a managed digital copy of an OpenCode config tree. This document is the source of truth for what the product does. The README is the story; this is the spec.

## The three verbs

Relit holds one portable copy of the tree and does three things:
...
Provider logins (`gh`, `az`, model `/connect`, local env files) stay on the machine. Relit never performs them and never stores their tokens.

The first capture flips the source of truth: from then on Relit is the copy and the disk is a render. Any edit made directly on disk afterward is drift, resolved by an explicit conflict choice. Never a silent merge.

## The copy

| Layer | Lives in Relit | Lives on the machine |
| --- | --- | --- |
...
| `relit.json` manifest (schema version, min OpenCode) | yes | carried with the copy, not installed |
...
`relit.json` sits at the copy root and records:

- `schemaVersion`: the shape of the copy itself, so future Relit versions can migrate it.
...
- **Conflict.** If disk and Relit diverged after the first capture, apply prompts for an explicit choice: capture the disk into the copy, or apply the copy over the disk. No silent merge.
- **Idempotence.** Re-apply updates files from the copy and never clobbers the local env file.
- **Refusals.** Apply refuses when committed files hold secret shapes, when the copy is below the manifest floor, and when the Relit copy is missing (for wipe).

## Wipe

Wipe clears the machine only. It is scoped strictly: deletes `~/.config/opencode` and named overlay targets, nothing else. It refuses when the Relit copy is missing. The Relit revision and files always stay.

## The starter tree

The default copy is stack-agnostic:

- `relit.json` (manifest: schema version, minimum OpenCode version)
...
Relit must not ship job-specific agents (ERP, a named cloud, a named distro) as defaults.

## Acceptance criteria

The definition of done.

1. Relit holds a complete OpenCode tree that is not tied to one workplace stack.
2. No secret shape ever appears in committed copy files; capture strips live keys into `{env:NAME}` references.
3. Apply refuses a copy whose committed files still hold secret shapes, and when disk and Relit have diverged it prompts for an explicit choice, never a silent merge.
...
6. Wipe deletes only the machine-side tree, never the Relit copy.
...
- **Storage.** Local Relit state vs a Git repo the user already owns. Git is the likely v1 persistence; Relit is the editor and apply tool, not a new forge.
- **Project overlays.** One global tree first, then optional named project packs. How a pack maps to a project is open.
- **MCP servers.** Definitions belong in the copy; credentials do not. How far Relit validates MCP schemas is open.
