...
- **Storage.** Git repo the user owns vs local Relit state. Git is the likely v1 persistence. Decided before v0.1 ships.
- **MCP validation depth.** How far apply validates MCP server definitions. Decide before v0.2.

## v0.1: the CLI exists

The CLI is the product gate. Without it, Relit is a model, not a tool.

Scope:

- `relit init`: create the starter tree (manifest, config, rules, starter agents/skills/command).
- `relit capture`: read `~/.config/opencode` and overlays, strip secrets into `{env:NAME}`, write the copy. First capture flips the source of truth.
- `relit edit`: open the copy as the source of truth; saving redacts known secret shapes; core files are protected from removal.
- `relit apply`: write the copy to `~/.config/opencode`; first run prompts for unset `{env:NAME}` values and writes them to the local env file; refuses a secret-dirty copy; refuses below the manifest floor; prompts on divergence.
- `relit wipe`: scoped machine-side delete, refuses when the copy is missing.
